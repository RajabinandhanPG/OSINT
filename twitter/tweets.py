...
def __user_input() -> str:
    parser = argparse.ArgumentParser(description='Scrape User Tweets')
...
def get_user_details(username: str) -> dict:
    url = f"https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name={username}"
    headers = {
...
if __name__ == '__main__':
    ...
    if user_details:
        ...
        filename = f"twitter/users/{username}_Tweets.json"
        try:
